...
// @ts-ignore - Lodash type declaration
...
const ProductItem = memo(({ 
    ...
    // 🚀 PERFORMANCE: Custom comparison - only re-render if product-related props change
    return (
        prevProps.product === nextProps.product &&
        prevProps.productIndex === nextProps.productIndex &&
        prevProps.user === nextProps.user &&
        prevProps.expandedComponents[prevProps.productIndex] === nextProps.expandedComponents[nextProps.productIndex] &&
        prevProps.products.length === nextProps.products.length
    );
});

// 🚀 PERFORMANCE: Memoized Payment Terms to prevent product re-renders
const PaymentTermsSection = memo(({ 
    paymentTerms, 
...
const PreventiveForm: FC<{ activeRevisionLabel: string | null, revisionData: DocumentDataDataType, user?: UserType }> = memo(({ activeRevisionLabel, revisionData }) => {
    ...
    useEffect(() => {
        if (revisionData) {
            setUpdatedDocumentData((prevData: any) => ({
                ...prevData,
                data: revisionData,
            }));
        }
    ...
    useEffect(() => {
        if (updatedDocumentData?.data?.paymentTerms) {
            ...
            setLocalPaymentTerms(updatedDocumentData.data.paymentTerms);
        }
    }, [updatedDocumentData]);
    ...
    // 🚀 PERFORMANCE: Separate payment terms state to prevent main form re-renders
    const [localPaymentTerms, setLocalPaymentTerms] = useState(paymentTerms);
    
    // 🚀 PERFORMANCE: Debounce payment terms updates to prevent excessive re-renders
    const debouncedPaymentUpdate = useCallback(
        debounce((value: string) => {
            updateNestedDocumentField(['data', 'paymentTerms'], value);
            setPaymentTerms(value); // Update the main state only after debounce
        }, 500),
        [updateNestedDocumentField]
    );
